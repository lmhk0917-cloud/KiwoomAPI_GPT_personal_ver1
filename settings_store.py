...
DEFAULT_SETTINGS = [
    ...
    (
        ...
        "bool",
        config.ENABLE_FORCE_GPT_INTRADAY_EVENT,
        "Force GPT review when a sudden intraday move has enough confirmations.",
    ),
    (
        "FORCE_GPT_RETURN_1BAR_PCT",
        "float",
        config.FORCE_GPT_RETURN_1BAR_PCT,
        "Minimum absolute 1-bar return percentage for forced GPT intraday review.",
    ),
    (
        "FORCE_GPT_VOLUME_RATIO",
        "float",
        config.FORCE_GPT_VOLUME_RATIO,
        "Minimum volume ratio used as confirmation for forced GPT intraday review.",
    ),
    (
        "FORCE_GPT_MIN_CONFIRMATIONS",
        "int",
        config.FORCE_GPT_MIN_CONFIRMATIONS,
        "Minimum confirmation count required before forced GPT intraday review.",
    ),
    (
        "GPT_FORCE_EVENT_TYPES",
        "json",
        config.GPT_FORCE_EVENT_TYPES,
        "Event types that bypass score filtering and always qualify for GPT analysis.",
    ),
    (
    ...
    (
        ...
        "bool",
        config.ENABLE_INTRADAY_NEWS_CONTEXT,
        "Fetch low-weight news context when unusual intraday events occur.",
    ),
    (
        "NEWS_CONTEXT_COOLDOWN_SEC",
        "int",
        config.NEWS_CONTEXT_COOLDOWN_SEC,
        "Per-symbol cooldown for intraday news context fetches.",
    ),
    (
        "NEWS_CONTEXT_TIMEOUT_SEC",
        "int",
        config.NEWS_CONTEXT_TIMEOUT_SEC,
        "HTTP timeout for intraday news context fetches.",
    ),
    (
        "NEWS_CONTEXT_MAX_ITEMS",
        "int",
        config.NEWS_CONTEXT_MAX_ITEMS,
        "Maximum news RSS items kept for GPT context.",
    ),
    (
        "NEWS_CONTEXT_RSS_URL_TEMPLATE",
        "str",
        config.NEWS_CONTEXT_RSS_URL_TEMPLATE,
        "RSS search URL template. It must include {query}.",
    ),
    (
        "NEWS_CONTEXT_TRIGGER_EVENT_TYPES",
        "json",
        config.NEWS_CONTEXT_TRIGGER_EVENT_TYPES,
        "Event types that trigger a low-weight intraday news context refresh.",
    ),
    (
